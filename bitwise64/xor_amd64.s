...
avx_loop:
    ...
    VPXOR   Y0, Y1, Y0          // Y0 = a[i] ^ b[i] for all 4 elements
...
TEXT ·xorAVX512(SB),NOSPLIT,$0-48
    MOVQ    a_base+0(FP), SI     // SI = pointer to a[0]
    MOVQ    b_base+24(FP), DI    // DI = pointer to b[0]
    MOVQ    a_len+8(FP), CX      // CX = length of slices
...
avx512_loop:
    ...
    VPXORQ  Z0, Z1, Z0          // Z0 = a[i] ^ b[i] for all 8 elements
