...
func init() {
	...
	for i := 10; i < 1e9; i *= 10 {
		...
		stagesNotBytes = append(stagesNotBytes, stageBytes{a, b, res})
	}
}
...
func TestNotBytes(t *testing.T) {
	testfnNotBytes := func(t *testing.T, stages []stageBytes, fn func([]byte)) {
		for i := 0; i < len(stages); i++ {
			st := &stages[i]
			t.Run(strconv.Itoa(len(st.a)), func(t *testing.T) {
	...
	testfnNotBytes(t, stagesNotBytes, NotBytes)
}
...
func BenchmarkNotBytes(b *testing.B) {
	benchfnNotBytes := func(b *testing.B, stages []stageBytes, fn func([]byte)) {
		for i := 0; i < len(stages); i++ {
			st := &stages[i]
			b.Run(strconv.Itoa(len(st.a)), func(b *testing.B) {
	...
	benchfnNotBytes(b, stagesNotBytes, NotBytes)
}
