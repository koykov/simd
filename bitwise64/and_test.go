...
func init() {
	...
	for i := 10; i < 1e9; i *= 10 {
		...
		stagesAndBytes = append(stagesAndBytes, stageBytes{a, b, res})
	}
}
...
func TestAndBytes(t *testing.T) {
	testfnBytes(t, stagesAndBytes, AndBytes)
}
...
func BenchmarkAndBytes(b *testing.B) {
	benchfnBytes(b, stagesAndBytes, AndBytes)
}
